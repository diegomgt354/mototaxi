package com.idat.mototaxi.services;
...
import com.idat.mototaxi.activities.driver.NotificationBookingActivity;
import com.idat.mototaxi.channel.NotificationHelper;
import com.idat.mototaxi.receivers.AcceptReceiver;
import com.idat.mototaxi.receivers.CancelReceiver;
...
public class MyFirebaseMessagingClient extends FirebaseMessagingService {
        ...
        if(title != null){
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
                if(title.contains("SOLICITUD DE SERVICIO")){
                    ...
                    showNotifiactionAPIOreoActions(title, body, idClient);
                    ...
                    manager.cancel(2);
                    showNotifiactionAPIOreo(title, body);
                }else {
                    showNotifiactionAPIOreo(title, body);
                }
            }else{
    ...
    private void showNotificationActivity(String idClient, String origin, String destination, String distance, String min) {
        ...
        boolean isScreenOn = pm.isScreenOn(); //isScreenOn esta obsoleto para android 4.4w a mas XD
        if(!isScreenOn){
    ...
    //Mostrar notificacion version "Oreo" a mas
    @RequiresApi(api = Build.VERSION_CODES.O)
    private void showNotifiactionAPIOreo(String title, String body) {
    ...
    //Mostrar notificacion versiones inferiores
    ...
    @RequiresApi(api = Build.VERSION_CODES.O)
    private void showNotifiactionAPIOreoActions(String title, String body, String idClient) {
